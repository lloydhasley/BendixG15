...
class G15Sim:
    ...
    def __init__(self, emulator, Verbosity=0, WatchDog=90000000):
        ...
        # track host OS to make minor changs in operation, if any
        ...
        #
        gui_type = self.emulator.gui_type
        if gui_type == 'curses':
            # curses has non-block input checking (at least on Darwin)
            # so putting execution code into 2nd thread is not needed
            threaded=0
        else:
            threaded=0

        self.sim_start(threaded)

    def sim_start(self, threaded):
        ...
        #   note: GUI needs to be the foreground thread
        ...
        while True:
            ...
            if (self.peek_next_event() == 'cpu') and (self.number_instructions_to_execute == 0):
                ...
                if self.number_instructions_to_execute > 0:
                    if self.g15.cpu.halt_flag:
                        print("CPU is halted")
                        self.number_instructions_to_execute = 0
                        print("Clearing Halt flag")
                        print(" instruction count=", self.g15.cpu.total_instruction_count)
                        self.g15.cpu.halt_flag = 0           # python model auto clears halt_flag
                        continue
