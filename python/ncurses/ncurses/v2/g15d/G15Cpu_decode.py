...
class g15d_decode:
    ...
    def decode(self, instruction):
        """
        Split a instruction into its fields
    ...
    def split(self, instruction):
        ...
        if source in self.source_special_dict:
            ...
            source_special = ''
        #
        # determine if special instruction
        # and decode it it is
        #
        destination_special = ''
        if destination == 31:
        ...
        instruction['ts'] = (not instruction['dp']) and (not instruction['divide'])
    ...
    def determine_start_end_times(self, instruction):
        ...
        if instruction['loc'] == 107:
            # 1940 correction is applied during WT=107
            # so if mark is at 107, then result is 20 less than specified
            instruction['next_cmd_word_time'] = Minus_20_Adjust(instruction['next_cmd_word_time'] )
            if instruction["deferred"]:
                time_start = Minus_20_Adjust(time_start)
            time_end = Minus_20_Adjust(time_end)
