...
ascii_to_code = {		# device: 2=paper tape, 1=typwriter
    ...
    '\n': {'code': 0x02, 'devices': IO_DEVICE_TYPEWRITER | IO_DEVICE_PAPER_TAPE},
...
def code_2_ascii(code):
    ...
    if (devices & IO_DEVICE_TYPEWRITER) != 0:
        return ascii
...
def signmag_to_str(signmag, width=7):
    ...
    ret_str = sign_str + mag_to_str(mag, width)
    return ret_str


def mag_to_str(num, width=7):
...
# create an event
#
# function in subr.py since sim will not yet exist when G15 is created
#
def create_event(name, command):
    ''' create a simulator event queue entry '''
