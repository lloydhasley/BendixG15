...
#            Byte = ord(bytestr)
        ...
        for block in self.tape_parsed:
            ...
            # replace checksum algorithm with Rob's
            for word in block:
                value = word >> 1
                sign = word & 1
                if sign:
    ...
    def checksum(self, block):
        ...
        for i in j:
            ...
            if self.verbosity & VERBOSITY_PT_CHECKSUM:
                str1 = int_to_str(sum)
                print('i=', i, 'word value (+-dec)=', value, '/value-x=', int_to_str(value), 'temp checksum=', str1)
                word = int_to_signmag(sum)
                str1 = signmag_to_str(word)
                print(str1)
    ...
    def numbertrack_word(seed, position):
        ...
        :param seed:            base word
        :param position:        word time
        :return:
        '''
        word = seed
        word |= position << 21
