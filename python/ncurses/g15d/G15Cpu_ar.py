...
#        if late_bus == 1:       # not documented in Theory of Operation Document (???)/433
#            late_bus = 0
        ...
        if self.cpu.verbosity & G15Cpu.VERBOSITY_CPU_AR:
            ...
            #print 'wt=%3d'%wt, 'old sum wus=%08x'%(ar ^ late_bus_sign), ' wucs=%08x'%(result^carry), ' wcs=%08x'%result
            # print 'wt=%3d'%wt,' us=', ar_sign ^ late_bus_sign, ' cs=', sum_sign, 'sum=%08x'%result, 'lb=%08x'%late_bus, 'oar=%08x'%ar
            outstr = "wt=%3d: " % wt
        ...
        # print '%08x'%late_bus
        # print 'wt=%3d'%wt, 'old sum w=%08x'%ar, ' sign=',(late_bus_sign ^ ar_sign), ' wcus=%08x'% (ar ^ late_bus_sign), 'res-cs%08x'%result
