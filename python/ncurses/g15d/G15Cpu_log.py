...
class g15d_log:
    ...
    def logger(self, time_start, time_end, early_bus, intermediate_bus, late_bus):
        ...
        str2 = "%3d" % instruction['loc']
        str3 = "%5d" % self.cpu.word_time_rollover(time_start)
        str4 = "%5d" % self.cpu.word_time_rollover(time_end)
        str5 = "%3d" % cmd_line_map[instruction['cmd_line']]
        str6 = "%4d" % instruction['loc']
        # str7 = instr_2_hex_string(self.instruction['instr'])
        str7 = "%08x" % self.cpu.instruction['instr']
        ...
        if instruction['deferred']:
            ...
            str8 = "  I"
        str9 = "%3d" % instruction['t']
        str10 = "%3d" % instruction['n']
        ...
        str13 = "%08x" % early_bus
        str14 = "%08x" % intermediate_bus
        str15 = "%08x" % late_bus

        str16 = "%08x" % self.g15.drum.read(AR, 0, 0)
        ...
        if self.fout is not None:
            if (self.fout_count % 10) == 0:
                print >>self.fout, self.header
            print >>self.fout, lstr1 + lstr2 + lstr3
            self.fout.flush()
