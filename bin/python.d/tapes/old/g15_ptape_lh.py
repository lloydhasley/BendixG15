...
class PaperTape(g15_ptape_subr, g15_ptape_verilog, g15_ptape_dis):
	def __init__(self, Debug=0):
		...
		self.PTI_CHAR_SET = "0123456789uvwxyz-DTSR"
	...
	def WritePt(self, file, reverse=0):
		...
		if reverse:
			image_reverse = self.ImageReverse( self.Image )
			fout.write( bytearray( image_reverse ) )
		else:
	...
	def ReadPti(self, file):		
		...
		for line in fin:
			...
			if (lastchar != 'R') and (lastchar != 'S'):
				errflag = 1
				if self.debug & 2:
					print 'invalid last char: ',lastchar, ' in line: ',line
				continue
			...
			for c in line:
				...
				if ii < 16:
					...
					quadword |= sign
					sign = 0
				elif (c == 'R') or (c == 'S'):	# /==R==reload, s==stop
					# add to block
